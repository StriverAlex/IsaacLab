...
class OVRTXRenderer(BaseRenderer):
    ...
    def _close_legacy(self) -> None:
        """Release the renderer's tensor bindings and stage."""
    ...
    def cleanup(self, render_data: OVRTXRenderData | None) -> None:
        """Release render-data buffers without disturbing shared renderer state."""
    ...
    def close(self) -> None:
        """Release shared stage state. See :meth:`~isaaclab.renderers.base_renderer.BaseRenderer.close`."""
        if self._use_ovstage:
            self._close_ovstage()
        else:
            self._close_legacy()
    ...
    def _close_ovstage(self) -> None:
        """Release the renderer's stage queries, path lists and ovstage stage."""
        ...
        #
        # Both are guarded because close can run before initialization completed and must remain
        # idempotent. detach_ovstage() raises when nothing is attached, and the ExitStack does not
        # exist until _initialize_from_spec_ovstage creates it.
