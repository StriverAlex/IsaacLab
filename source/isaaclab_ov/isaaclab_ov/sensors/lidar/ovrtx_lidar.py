...
class OVRTXLiDAR(SensorBase):
    ...
    def prepare_renderer(self, renderer: BaseRenderer) -> None:
        ...
        for _source_root, _destination, source_path, env_ids in cloner.query.iter_sources(plan, self.cfg.prim_path):
            for env_id in env_ids:
                sensor_path = cloner.query.path_to_clone(plan, source_path, env_id)
                if sensor_path is None:
                    raise RuntimeError(f"ClonePlan could not resolve OVRTX LiDAR for environment {env_id}.")
    ...
    def _validate_authored_lidar_prims(self) -> None:
        ...
        matches = tuple(cloner.query.iter_sources(self._clone_plan, self.cfg.prim_path))
        if not matches:
